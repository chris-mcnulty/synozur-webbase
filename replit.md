...
- **Microsoft Entra SSO** (`artifacts/api-server/src/lib/entraOidc.ts` + `lib/entra.ts`, #126): native OIDC client (no Clerk). The OIDC flow uses authorization-code + PKCE against the Entra v2.0 endpoint, validates the ID token signature against the tenant JWKS via `jose`, mirrors tenant + object id onto the user row, resolves transitive group membership via Microsoft Graph, and reconciles `user_roles` against admin-managed mappings in `entra_group_role_mappings`. Roles previously sourced from a group the user no longer belongs to are removed on next sign-in — Entra offboarding instantly removes CMS access. Sessions are server-side rows (`sessions` table) keyed by an `sid` HttpOnly cookie carrying only the random session id (the row holds the rest, so any session can be revoked unilaterally). Configuration: `ENTRA_TENANT_ID`, `ENTRA_APP_CLIENT_ID`, `AUTH_REDIRECT_URI`, optional `ENTRA_APP_CLIENT_SECRET` (only needed for confidential web-app registrations), optional `ENTRA_APP_CLIENT_ID` + `ENTRA_APP_CLIENT_SECRET` with `GroupMember.Read.All` for app-only Graph token used by group resolution, optional `AUTH_POST_LOGOUT_URI`. Group → role mappings managed at `/admin/access/entra`.
