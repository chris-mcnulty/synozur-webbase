...
// `<kind>.<id>.<exp>` signed with HMAC-SHA256. We pick a reasonable secret
// precedence: PREVIEW_TOKEN_SECRET (explicit opt-in) ➜ SESSION_SECRET ➜
// CLERK_SECRET_KEY (always present on deployed envs) ➜ a boot-time random
// secret (dev fallback only; tokens don't survive restart, which is fine
// since they expire in 24 h anyway).
