...
// Narrow guard used for messages from APPROVED senders. It blocks only the
// kinds of machine mail that would cause a feedback loop or be meaningless as a
// briefing — namely true auto-REPLIES (out-of-office / vacation), auto-
// notifications, and Exchange-generated system messages (NDRs, read receipts,
// delivery reports).
//
// Unlike isAutoGeneratedEmail(), it deliberately does NOT reject
// `Auto-Submitted: auto-generated` or `X-Auto-Response-Suppress`. Legitimate
// briefings that are auto-forwarded or redirected by an Exchange rule preserve
// those markers from the *original* (machine-generated) briefing, so rejecting
// them would block exactly the content an approved sender intends to send.
//
// All checks are case-insensitive on both header names and values.
export function isAutoReplyOrSystemMessage(message: GraphMessage): boolean {
  const headers = message.internetMessageHeaders ?? [];
  const headerMap: Record<string, string> = {};
  for (const h of headers) {
    headerMap[h.name.toLowerCase()] = h.value;
  ...
  // RFC 3834 auto-replies / notifications (out-of-office, vacation, etc.).
  // Values look like "auto-replied" or "auto-notified" (optionally with params).
  // "auto-generated" is intentionally allowed through.
  const autoSubmitted = headerMap["auto-submitted"]?.toLowerCase().trim();
  if (
    autoSubmitted &&
    (autoSubmitted.startsWith("auto-replied") ||
      autoSubmitted.startsWith("auto-notified"))
  ) {
    return true;
  }

  // Exchange-generated system messages: NDRs, read receipts, OOF notices,
  // delivery reports. A user-redirected briefing is not marked this way.
  if ("x-ms-exchange-generated-message-by" in headerMap) {
    return true;
  }
