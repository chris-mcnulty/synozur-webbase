...
async function handleInboundMessage(
  ...
  // Drop machine-generated messages (auto-replies, OOF notices, NDRs,
  // delivery reports) even from approved senders — those are never
  // intentional briefings and must not loop back through the pipeline.
  //
  // Manual forwards (Fw:/Fwd: prefix) ARE intentional and must be allowed
  // through. Outlook/Exchange adds In-Reply-To even on forwards (threading),
  // so we must check the subject prefix first before calling isAutoGeneratedEmail
  // to avoid discarding legitimate user-forwarded content.
  const isFwd = /^\s*(fw|fwd)\s*:/i.test(message.subject ?? "");
  if (!isFwd && isAutoGeneratedEmail(message)) {
    logger.info(
      { sender, subject: message.subject, messageId },
      "Inbound briefing message is auto-generated; discarded",
    );
    return;
  }
