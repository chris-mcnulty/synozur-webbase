...
function serialize(c: CaseStudy) {
  return {
    ...
    atAGlance: c.atAGlance ?? null,
    whatWeDid: c.whatWeDid ?? null,
    whyItWorked: c.whyItWorked ?? null,
...
const CaseStudyBody = z.object({
  ...
  atAGlance: z.string().nullish(),
  whatWeDid: z.string().nullish(),
  whyItWorked: z.string().nullish(),
...
router.post("/cms/case-studies", ...adminGuard, async (req, res) => {
  ...
  const [row] = await db
    ...
    .values({
      ...
      atAGlance: d.atAGlance ?? null,
      whatWeDid: d.whatWeDid ?? null,
      whyItWorked: d.whyItWorked ?? null,
