...
| — | Bot prerender for synozur-owned paths (`/sprint`, `/proof`, `/fit`, `/book`, `/`, `/about`, `/faq`, `/events`, etc.) — the `react-vite` integratedSkill forces static production serving on the synozur artifact, so `server.mjs` never runs in prod and social bots (LinkedIn, Slack, Teams, X) receive the generic fallback OG card on these routes. Fix: add `/sprint`, `/proof`, `/fit`, `/book` (and any other high-value funnel paths) to the api-server's `artifact.toml` paths list and add a thin route that serves OG-prerendered HTML to social bots and the static SPA `index.html` to normal users — mirrors what `server.mjs` does but runs inside the api-server process which has no integratedSkill constraint. | SEO & Web-Platform | — |
