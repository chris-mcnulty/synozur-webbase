...
- [ ] **L2. Search Console + Bing Webmaster verification** → #160.
  ...
  **Code shipped:** the SPA server (`artifacts/synozur/server.mjs`) splices both meta tags into the bare HTML response from `GOOGLE_SITE_VERIFICATION` / `BING_SITE_VERIFICATION` env vars at boot. **Remaining ops:** set those two env vars in production, confirm DNS TXT, and submit the sitemap from each console.
