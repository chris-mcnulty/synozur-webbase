...
/admin/site-config/list-page-copy — List page hero copy per content section + Sprint funnel share images
...
| `/admin/site-config/list-page-copy` | Edit hero/intro copy for each content-listing section, plus the SEO title/description and social share image for the Sprint funnel pages (`/sprint`, `/proof`, `/fit`, `/book`) |
...
> **Adding a new list page section**: `content_parent_pages` is seeded from a fixed `SLUGS` array in `seedContentParentPages.ts` (and mirrored by an idempotent startup-migration step so production seeds itself). There is no admin "create" flow — add the slug to that array (and the migration step) and re-run the seed.
>
> **Sprint funnel share images**: the `sprint`, `proof`, `fit`, and `book` rows are not list pages — they exist so admins can override the OG image / SEO title / description that `ogResolver.ts` serves to social crawlers for those hand-coded routes. Empty fields fall back to the hardcoded `STATIC_PAGE_OG` defaults (generated by `artifacts/synozur/scripts/generate-og-images.sh`).
